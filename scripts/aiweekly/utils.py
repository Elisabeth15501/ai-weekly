...
def _probe(url: str, timeout: int = 8) -> bool:
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": _UA})
        with _build_opener().open(req, timeout=timeout) as r:
            r.read(256)
        return True
    except Exception:
        return False
...
def _detect_region() -> str:
    ...
    """
    cn_ok = _probe("https://www.baidu.com", 6) or _probe("https://rank.opencompass.org.cn/leaderboard-llm", 6)
    foreign_ok = _probe("https://lmarena.ai/leaderboard", 8) or _probe("https://huggingface.co", 6)
